...
def create_index_data():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS index_basic
    		(ts_code VARCHAR(20),name TEXT,market VARCHAR(20),publisher VARCHAR(20),category VARCHAR(20),
    		base_date DATE,base_point DECIMAL(16,6),list_date DATE
    		);
    		""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS index_daily
    		(ts_code VARCHAR(20),trade_date DATE,close DECIMAL(16,6),open DECIMAL(16,6),high DECIMAL(16,6),
    		low DECIMAL(16,6),pre_close DECIMAL(16,6),`change` DECIMAL(15,6),pct_chg DECIMAL(13,6),vol DECIMAL(21,6),
    		amount DECIMAL(21,6)
    		);
    		""")
    cursor.execute("""CREATE TABLE IF NOT EXISTS index_weekly
    		(ts_code VARCHAR(20),trade_date DATE,close DECIMAL(16,6),open DECIMAL(16,6),high DECIMAL(16,6),
    		low DECIMAL(16,6),pre_close DECIMAL(16,6),`change` DECIMAL(15,6),pct_chg DECIMAL(13,6),vol DECIMAL(24,6),
    		amount DECIMAL(25,6)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS index_monthly
    		(ts_code VARCHAR(20),trade_date DATE,close DECIMAL(16,6),open DECIMAL(16,6),high DECIMAL(16,6),
    		low DECIMAL(16,6),pre_close DECIMAL(16,6),`change` DECIMAL(15,6),pct_chg DECIMAL(13,6),vol DECIMAL(25,6),
    		`amount` DECIMAL(26,6)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS index_weight
    		(index_code VARCHAR(20),con_code VARCHAR(20),trade_date DATE,weight DECIMAL(14,6)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS index_dailybasic
    		(ts_code VARCHAR(20),trade_date DATE,total_mv DECIMAL(26,6),float_mv DECIMAL(26,6),
    		total_share DECIMAL(25,6),float_share DECIMAL(25,6),free_share DECIMAL(25,6),turnover_rate DECIMAL(13,6),
    		turnover_rate_f DECIMAL(13,6),pe DECIMAL(14,6),pe_ttm DECIMAL(14,6),pb DECIMAL(13,6)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS index_classify
    		(index_code VARCHAR(20),industry_name VARCHAR(20),level VARCHAR(20),industry_code VARCHAR(20),
    		is_pub VARCHAR(20),parent_code VARCHAR(20),src VARCHAR(20)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS index_member
    		(index_code VARCHAR(20),con_code VARCHAR(20),in_date DATE,out_date DATE,is_new VARCHAR(20)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS daily_info
    		(trade_date DATE,ts_code VARCHAR(20),ts_name TEXT,com_count INT,total_share DECIMAL(17,6),
    		float_share DECIMAL(17,6),total_mv DECIMAL(18,6),float_mv DECIMAL(18,6),amount DECIMAL(16,6),
    		vol DECIMAL(15,6),trans_count DECIMAL(13,6),pe DECIMAL(14,6),tr DECIMAL(13,6),exchange VARCHAR(20)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS sz_daily_info
    		(trade_date DATE,ts_code VARCHAR(20),count INT,amount DECIMAL(24,6),vol DECIMAL(18,1),total_share DECIMAL(25,6),
    		total_mv DECIMAL(26,6),float_share DECIMAL(25,6),float_mv DECIMAL(26,6)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS ths_index
    		(ts_code VARCHAR(20),name TEXT,count DECIMAL(16,6),exchange VARCHAR(20),list_date DATE,type VARCHAR(20)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS ths_daily
    		(ts_code VARCHAR(20),trade_date DATE,close DECIMAL(20,6),open DECIMAL(20,6),high DECIMAL(20,6),
    		low DECIMAL(20,6),pre_close DECIMAL(20,6),avg_price DECIMAL(15,6),`change` DECIMAL(19,6),
    		pct_change DECIMAL(14,6),vol DECIMAL(21,6),turnover_rate DECIMAL(14,6)
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS ths_member
    		(ts_code VARCHAR(20),code VARCHAR(20),name TEXT
    		);
    		""")

    cursor.execute("""CREATE TABLE IF NOT EXISTS index_global
