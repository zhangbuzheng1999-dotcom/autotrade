"""
这个回测引擎与正常引擎不同，普通的事件引擎是并行的：事件接收后会并行交由handle处理同时接收下一事件。Backtest Event Engine是串行的,
处理完上一事件才会接收新的事件。这一做法是为了防止在回测中，这一根k线还没处理完，下一根k线已经传过来了
"""
...
from autotrade.engine.event_engine import EVENT_TIMER
