...
class CNStockPriceSpec(BaseRQSpec):
    RESOURCE_NAME = "cn_stock_price"
    ...
    DB_QUERY_FIELDS = {
        ...
        "contract_multiplier", "iopv", "day_session_open", "adjust_type",
    }
    ...
    def resolve_db_filter_specs(self, filters: dict[str, Any]) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "adjust_type": {"column": "adjust_type", "op": "eq"},
        }
    ...
    def normalize_df(
        ...
        result = df
        result["adjust_type"] = (filters or {}).get("adjust_type", "none")
        return result
...
class CNStockInstrumentSpec(BaseRQSpec):
    """
    ...
    def validate_filters(self, filters: dict[str, Any], mode: FetchMode) -> None:
        ...
        if market not in {"cn", "hk"}:
            raise ValueError("cn_stock_instruments market must be one of {'cn', 'hk'}")

    def resolve_database(self, filters: dict[str, Any]) -> str:
        return "rq_stock_data"

    def resolve_table(self, filters: dict[str, Any]) -> str:
        return "cn_stock_instruments"
    ...
    def normalize_df(
        ...
        if "order_book_id" not in result.columns:
            raise ValueError("cn_stock_instruments dataframe missing order_book_id column")
        if result["order_book_id"].isna().any():
            bad_rows = result[result["order_book_id"].isna()]
            raise ValueError(
                f"cn_stock_instruments dataframe contains null order_book_id rows: {len(bad_rows)}"
            )
