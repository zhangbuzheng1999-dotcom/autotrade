...
class ETFPriceSpec(BaseRQSpec):
    RESOURCE_NAME = "etf_price"
...
class ETFInstrumentSpec(BaseRQSpec):
    """
    all_instruments(type='ETF')
    ...
    def validate_filters(self, filters: dict[str, Any], mode: FetchMode) -> None:
        ...
        if market not in {"cn", "hk"}:
            raise ValueError("etf_instruments market must be one of {'cn', 'hk'}")
    ...
    def normalize_df(
        ...
        if "order_book_id" not in result.columns:
            raise ValueError("etf_instruments dataframe missing order_book_id column")
        if result["order_book_id"].isna().any():
            bad_rows = result[result["order_book_id"].isna()]
            raise ValueError(
                f"etf_instruments dataframe contains null order_book_id rows: {len(bad_rows)}"
            )
